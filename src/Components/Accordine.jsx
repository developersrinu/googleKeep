...
const Accordine = () => {
  ...
  }, [filterItemName, list]); // Include 'list' as a dependency to avoid the infinite loop
    ...
    <div style={{ textAlign: 'center' }}>
      <div className='cord'>
        ...
        {full && (
          <div className='aBot'>
            ...
            <div className='items-main'>
              ...
              <div data-tip="Notification"><FaBell /></div>
              <div data-tip="User"><FaUser /></div>
              <div data-tip="Palette"><FaPalette /></div>
              <div data-tip="Medal"><FaMedal /></div>
              <div data-tip="Image"><FaRegImage /></div>
              <div data-tip="Options"><FaEllipsisV /></div>
              <div data-tip="Previous"><FaChevronLeft /></div>
              <div data-tip="Next"><FaChevronRight /></div>


              {isEdit ? (
                <div className='button-18' onClick={save}>Save</div>
              ) : (
                <div className='button-18' onClick={close}>Close</div>
              )}
            </div>
          </div>
        )}
       ...
       <div className={isFlag ? 'listDiv' : 'GridDiv'}>
        {list.map((v, i) => (
          <div className={isFlag ? 'list-item' : 'Grid-item'} key={i}   style={{ backgroundColor: v.color }}>

            <div style={{ fontSize: '1.2rem' }}>{v.titleName}</div>
            <div style={{ textOverflow: 'ellipsis' }}>{v.descName}</div>


            <div style={{ display: 'flex', justifyContent: 'space-evenly', marginTop: '20px' }} className='childItem' >
              <div className='aa'><FaBell /></div>
              <div className='aa'><FaUser /></div>

             

              <div class="color-picker-container">
                <input type="color" class="color-input" id="colorInput" onChange={(e)=>divColor(i,e.target.value)} />
              ...
              <div className='aa'><FaMedal /></div>
              <div><FaRegImage /></div>
              <div className='aa'><FaEllipsisV /></div>
              <div><FaRegTrashAlt className='delete' onClick={() => remove(i)} /></div>
              <div><FaEdit className='delete' onClick={() => edit(i)} /></div>
            </div>
